...
func init() {
	...
	complyCmd.Flags().StringP("config", "c", "", "Path to argus.yml (or appsec.yml) configuration file")
...
By default it scans the target path (same scanners as 'argus scan', without
...
func savedRunFindings(target, runID string) ([]model.Finding, string, error) {
	...
	if runID == "" {
		...
		if len(runs) == 0 {
			return nil, "", fmt.Errorf("no saved runs under %s (run `argus scan %s --save` first)", store.Dir, target)
		}
